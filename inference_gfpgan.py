...
def main():
    ...
    parser.add_argument('--test_path', type=str, default='D:/video/dataset', help='Input folder')
    ...
    parser.add_argument('--save_root', type=str, default='HQ_dataset', help='Path to save root')
    parser.add_argument(
        '--ext',
        type=str,
        default='auto',
    ...
    for datadir in os.listdir(args.test_path):
        datapath = os.path.join(args.test_path, datadir)
        print(f'Looking into dir{datapath}...')
        img_list = sorted(glob.glob(os.path.join(datapath, '*.png')))
        json_list = sorted(glob.glob(os.path.join(datapath, '*.json')))
        for img_path in img_list:
            ...
            if restored_img is not None:
                ...
                if args.suffix is not None:
                    save_restore_path = os.path.join(args.save_root, datadir,
                                                    f'{basename}_{args.suffix}.{extension}')
                else:
                    save_restore_path = os.path.join(args.save_root, datadir, f'{basename}.{extension}')
                print(save_restore_path)
                imwrite(restored_img, save_restore_path)
        for json_path in json_list:
            json_name = os.path.basename(json_path)
            print(f'Processing {json_name} ...')
            basename, ext = os.path.splitext(json_name)
            if args.ext == 'auto':
                extension = ext[1:]
            else:
                extension = args.ext
            if args.suffix is not None:
                ...
                save_restore_path = os.path.join(args.save_root, datadir, f'{basename}.{extension}')
            shutil.copyfile(json_path, save_restore_path)
